...
def solve_unit_conversion(prompt: str) -> str | None:
    ...
    if has_c and has_f:
        ...
        if want_f and not want_c:
            return _fmt(round(N * 9.0 / 5.0 + 32.0, 4))
        if want_c and not want_f:
            return _fmt(round((N - 32.0) * 5.0 / 9.0, 4))
        return None
    if has_c or has_f:
...
def solve_day_of_week(prompt: str) -> str | None:
    ...
    for m in re.finditer(r"\byesterday\s+(?:was|is)\s+(" + _WD_ALT + r")\b", p):
        todays.add((_WD_INDEX[m.group(1)] + 1) % 7)
    for m in re.finditer(r"\btomorrow\s+(?:is|will be)\s+(" + _WD_ALT + r")\b", p):
        todays.add((_WD_INDEX[m.group(1)] - 1) % 7)
    if len(todays) != 1:
